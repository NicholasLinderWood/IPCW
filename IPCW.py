...
class IPCW:
    ...
    def CalculateSurvivalByMELD(self):
        ...
        for m in self.meld2dfB:
            print(f'Calculating survival for candidates at {self._meld_col} {int(m)}')
            self.CalculateSurvival(m, self._t)
    ...
    def CalculateSurvival(self, meld, t):
        '''Calculate survival for all candidates at the given MELd score up to time t.'''

        #Read in the A File
        dfA = self.dfA

        #grab the B file for this MELD
        dfB_meld = self.meld2dfB[meld]
    ...
    def Plot(self, m, show = True):
        ...
        ax.plot(T, S, 'r-', linewidth = w, zorder = 2, label = f'MELD-Na {m}')
